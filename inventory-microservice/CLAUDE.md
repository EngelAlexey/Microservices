...
- **`scrape_services.py`** — Obtiene el HTML de páginas de producto y reduce a JSON-LD + texto del body antes de mandarlo a la IA (`extract_relevant_content`). `scrape_product_page` usa **Cloudflare Browser Rendering** (render JS, vía `render_services.py`) cuando hay credenciales, con **fallback a `requests`** si Cloudflare no está configurado o falla. Esto permite scrapear **cualquier sitio**, incluidos los SPA/catálogos JS (Construplaza usa Algolia; El Lagar es SPA) que con `requests` solo devolvían el cascarón. `extract_product_image` saca la imagen real del producto en cascada (og:image → `<img>` → `background:url(...)` del `div.lupa`) **descartando logos/placeholders** (la causa raíz del bug histórico de guardar el logo era tomar el `og:image`, que en varias tiendas es el logo).
- **`render_services.py`** — Cliente de **Cloudflare Browser Rendering** (REST API). `render_content(url)` devuelve el HTML post-JS; `render_markdown(url)` devuelve Markdown limpio (reutilizable por otros servicios —p.ej. Kaizen AI— para leer/buscar en la web y pasar texto a un LLM). Cuota Free: 10 min de navegador/día (suficiente para pruebas). Requiere `CF_ACCOUNT_ID` + `CF_API_TOKEN` (permiso *Browser Rendering – Edit*).
- **`suppliers.py`** — **Capa de estandarización del scraping** (resolvers de producto por proveedor/plataforma). `resolve_supplier(url)` recorre una cascada y devuelve datos limpios (esquema de `extract_product_from_html` + `image_url`/`barcode`/`cabys`) o `None` (el caller cae al genérico):
  1. **Resolver dedicado por dominio** (`_RESOLVERS`) — para sitios con fuente propia mejor que scrapear. Hoy: **Construplaza** (su HTML no rinde ni con render headless porque carga por XHR de evento): consulta su índice **Algolia** (`Products`, credenciales search-only públicas del bundle `/bundles/RequeriredUtils`), elige el mejor hit por el slug, y trae itTitle/itModel/itSize (jerarquía por Gemini con datos limpios) + imagen real cloudfront + `barcode` + `cabys`.
  2. **Resolver de plataforma** (`_PLATFORM_RESOLVERS`) — una sola implementación cubre muchas tiendas. Hoy: **Shopify** (`<url>.json`, gate `/products/`) y **VTEX** (`/api/catalog_system/pub/products/search/{slug}/p`, gate URL terminada en `/p`; ej. Walmart CR/Más x Menos, Cemaco). Ambos traen título, marca, categoría, imágenes y barcode (ean/variant).
  3. Si nadie resolvió → **camino genérico** en el endpoint: Cloudflare render, luego `suppliers.resolve_from_html` (datos estructurados **schema.org/Product JSON-LD**: imagen + GTIN confiables si la página los trae) y, si no hay JSON-LD, extracción con IA sobre el HTML. Cubre **El Lagar** (su detalle SÍ entra al HTML renderizado) y la mayoría de SPAs. La jerarquía la da Gemini y la imagen `scrape_services.extract_product_image` (reglas determinísticas por dominio, p.ej. **El Lagar** → `Articulos_MED/{código}_MED.png` en alta resolución).

  El helper `_structure_with_ai` (usado por Construplaza y Shopify) resuelve la jerarquía con Gemini desde datos limpios. Extensible: agregar dominios a `_RESOLVERS`, plataformas a `_PLATFORM_RESOLVERS` y reglas de imagen a `scrape_services._DOMAIN_IMAGE_RULES`.
